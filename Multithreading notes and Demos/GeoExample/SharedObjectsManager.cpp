...
GeometricObject * SharedObjectsManager::load(string name) {
pthread_mutex_lock(&sync_mutex);
GeometricObject * obj = gobj[name]; // Fetch object based on name
if ( obj != NULL ) {
...
SharedObjectsManager::~SharedObjectsManager() // Destructor
{
// An iterator loop with a lambda function to delete all objects
for_each (gobj.begin(),gobj.end(),[](const std::pair<string,GeometricObject *>& it) -> bool {
 GeometricObject * obj = std::get<1>(it);
delete (obj);
 return true;
 });
