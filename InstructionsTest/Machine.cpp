...
void Machine::executeAll(){
  ...
  cout<< "-------------------------------------------" <<endl;
  cout<<"before: " << endl;
  for(next=0; next < code.size();++next){ //reads each element in file, mreads line per line using next.
    if(code[next].at(0) == "VAR"){
      VAR * obj = objMap[code[next].at(1)];
      obj = obj->clone(code[next]);
      varMap[code[next].at(2)] = obj;
