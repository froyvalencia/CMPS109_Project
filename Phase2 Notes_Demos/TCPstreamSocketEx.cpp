...
if ((sock = socket(AF_INET, SOCK_STREAM, 0)) == -1) terminate_with_error("Error CreatingSocket",sock);
...
setsockopt(sock,SOL_SOCKET,SO_REUSEADDR,(void *)&sock_opt,sizeof (sock_opt));
 ...
 bzero(&(serverAddr.sin_zero), 8); // We zero out the rest of the sockaddr_in to make it identical tosockaddr

 // Try to bind on the socket port, and if it fails we exit. Will fail if the socket is already acquired by
//another process
